...
│   │   │   └── SiteGuide.tsx              # 사이트 가이드
│   │   ├── UCIInfoModal.tsx               # 도시 편의성 지수 정보 모달
...
#### 상태 색상 토큰 (시민용 대시보드)
시민용 대시보드에서는 신호가 필요한 부분에만 제한적으로 색상을 사용합니다:

- **Success** (`--status-success-*`): 안정적, 완료, 감소(좋은 의미) - Green 계열
- **Warning** (`--status-warning-*`): 모니터링 중 - Yellow 계열 (파스텔 톤)
- **Attention** (`--status-attention-*`): 주의 필요 - Orange 계열 (파스텔 톤)
- **Danger** (`--status-danger-*`): 긴급/악화 - Red 계열 (파스텔 톤, 제한적 사용)
- **Info** (`--status-info-*`): 정보성 - Blue/Gray 계열

색상 토큰은 `src/styles/design-system.css`에서 CSS 변수로 정의되어 있으며, 모든 컴포넌트가 일관된 색상 체계를 사용합니다.
...
시민용 대시보드는 전체적으로 그린 계열 테마를 유지하면서, 신호가 필요한 부분에만 제한적으로 색상을 사용하여 정보를 명확하게 구분합니다.

#### 사이트 가이드
페이지 상단에 제공되는 단계별 가이드로, 대시보드 활용 방법을 안내합니다.

- 5단계 구성: 전체 추세 지표 → 지역별 현황 → 지역별 신호 추세 → 개선 현황 → 민원 신고
- 각 단계별 상세 설명 제공
- 도시 편의성 지수(UCI) 정보 모달 제공

#### 전체 추세 지표
도시 전역의 편의성 지수 변화 추이를 시각화합니다.

- 현재 도시 편의성 지수 및 전 분기 대비 변화율
- 변화 추세 아이콘 (▲ 증가, ▼ 감소, — 유지) 및 색상 표시
- 지속적 개선 추세 확인
- 분기별 추이 차트

#### 지역별 현황
구 단위 지역별 도시 편의성 상태를 지도로 표시합니다.

- Mapbox 기반 인터랙티브 지도
- 상태별 색상 구분 (개선 중, 안정적, 모니터링 중)
- 정확한 위치 정보는 공개되지 않음 (개인정보 보호)
- 지도 범례 제공

#### 지역별 신호 추세
구 단위 지역별 신호 변화 추이를 분석합니다.

- 지역별 상태 배지 (개선 중, 안정적, 모니터링 중, 주의 필요)
- 민원 신호 및 생활인구 신호 추세 표시
- 신호 변화 방향 색상 표시 (증가, 감소, 유지)
- 각 지역별 상태 설명 및 참고 사항

#### 개선 현황
진행 중인 도시 편의성 개선 사업 현황을 표시합니다.

- 상태별 분류: 개선 중, 안정적, 모니터링 중, 완료
- 상태 배지 색상 체계화 (안정적/완료: Green, 모니터링 중: Gray)
- 진행률 표시 (진행률 바)
- 최종 업데이트 날짜 및 완료일 표시
- 위치 정보 표시

#### 민원 신고 안내
도시 편의성과 관련된 문제를 간단한 절차로 신고할 수 있습니다.

- 3단계 신고 프로세스 안내
- 온라인 신고 방법 및 민원 포털 링크 제공
- 신고 시 참고 정보 가이드
...
#### 주요 색상 변수
- `--chateau-green-*`: Primary 색상 팔레트 (50~950)
- `--gray-*`: Neutral 색상 팔레트 (50~900)
- `--status-success-*`: 성공/안정 상태 (background, text, border, strong)
- `--status-warning-*`: 경고/모니터링 상태
- `--status-attention-*`: 주의 필요 상태
- `--status-danger-*`: 위험/긴급 상태 (제한적 사용)
- `--status-info-*`: 정보성 상태
...
시민용 대시보드는 다음 원칙에 따라 색상을 사용합니다:

1. **전체 테마 유지**: 그린 계열을 기본 테마로 유지
2. **제한적 색상 사용**: 신호가 필요한 부분에만 색상 적용
3. **파스텔/소프트 톤**: 과한 채도를 피하고 시민 친화적인 톤 사용
4. **빨강 제한**: 빨간색은 "진짜로 조치 필요"인 긴급/악화 상황에만 사용
5. **색상 토큰 기반**: 일관된 색상 체계를 위해 CSS 변수 사용
